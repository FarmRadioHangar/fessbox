...
function route(apiHandler, request, response) {
        ...
        if (typeof apiHandler[path] !== 'function') {
			...
			params.webUser = ip;
			/*
			console.error("DEBUG::cookie - " + request.headers.cookie);
			*/
			// cookies not used for now!
			// todo: optimize cookie parsing
			//var cookies = request.headers.cookie.split(/; */);
			/*
			var m;
			if (!params.regid) {
				if (request.headers.cookie && (m = /regid=([^ ,;]*)/.exec(request.headers.cookie))) {
					params.regid = m[1];
				} else {
					params.regid = request.headers['x-forwarded-for'];
				}
			}
			*/
